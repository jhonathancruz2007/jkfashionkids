...
export async function POST(req: Request) {
  try {
    ...
    // =========================================================
    // B. VALIDA OS PRODUTOS DO PEDIDO
    // =========================================================
    ...
    const produtosAusentes = produtoIds.filter((id) => !idsExistentes.has(id));

    if (produtosAusentes.length > 0) {
      return NextResponse.json(
        {
          error: "Um ou mais produtos do pedido não foram encontrados no banco de dados.",
          produtosAusentes,
        },
        { status: 400 }
      );
    ...
    const redirectUrl =
      ...
      `?orderId=${encodeURIComponent(String(pedidoCriado.id))}`;
