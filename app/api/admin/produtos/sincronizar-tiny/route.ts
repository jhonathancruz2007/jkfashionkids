...
type TinyStockResponse = {
  ...
  depositos?: Array<{ saldo?: number | null; reservado?: number | null; disponivel?: number | null }> | null;
};
...
async function getConfirmedStock(id: string): Promise<number> {
  let lastError: unknown = null;

  for (let attempt = 0; attempt <= STOCK_CONFIRM_MAX_RETRIES; attempt++) {
    try {
      const data = await getOlistV3<TinyStockResponse>(
        `/estoque/${encodeURIComponent(id)}`
      );
      const saldo = safeNonNegativeInt(data?.saldo);
      if (saldo == null) {
        throw new Error(`Olist/Tiny não retornou um saldo válido para ${id}.`);
      }
      return saldo;
    } catch (error) {
      lastError = error;
      const message = error instanceof Error ? error.message : String(error);
      const is429 = /API 429|HTTP 429|rate limit/i.test(message);
      if (!is429 || attempt >= STOCK_CONFIRM_MAX_RETRIES) break;
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`Não foi possível confirmar o estoque do produto Tiny ${id}.`);
}
...
async function updateExistingProduct(
  ...
  const hasVariations = aggregate.variationCount > 0;
  let tinyStock = aggregate.estoque;
  let stockSource: "product" | "variations" | "confirmed-stock" =
    hasVariations
      ? (safeNonNegativeInt(detail.estoque?.quantidade) != null &&
          (safeNonNegativeInt(detail.estoque?.quantidade) || 0) > 0
          ? "product"
          : "variations")
      : "product";
  let confirmed = false;
  ...
  return {
    ...
    stockSource,
    confirmed,
    matrixComplete: aggregate.matrixComplete,
  };
}
    ...
    const results = await mapWithConcurrency<SiteSyncEntry, BatchResult>(
      ...
      async (entry): Promise<BatchResult> => {
        try {
          ...
          console.warn(`[Olist V3] Falha no produto Tiny ${entry.tinyId}:`, errorMessage);
          return {
            kind: "failed",
            variationCount: 0,
            siteId: entry.siteId,
