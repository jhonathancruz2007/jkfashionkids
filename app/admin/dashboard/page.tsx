...
export default function PaginaDashboardAdmin() {
  ...
  // ============================================================
  // SINCRONIZAÇÃO DE ESTOQUE OLIST/TINY API V3
  // ============================================================
  // Este botão faz SOMENTE a leitura do estoque do Tiny e a atualização do
  // campo estoque no site. Não altera dados comerciais/cadastrais.
  const handleSincronizarTiny = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`O servidor não retornou JSON válido. HTTP ${prepareRes.status}`)
      }
      ...
      if (entries.length === 0) {
        exibirToast("Nenhum produto encontrado no site para sincronizar.", "error")
        return
      }

      const batchSize = Math.max(1, Math.min(18, Number(prepareData.batchSize) || 18))
      ...
      for (let inicio = 0; inicio < entries.length; inicio += batchSize) {
        ...
        while (tentativa < 3) {
          ...
          exibirToast(
            `Sincronizando estoque: lote ${loteNumero}/${totalBatches} • ${processados}/${entries.length} produtos...`
          )

          console.log(`=== ESTOQUE LOTE ${loteNumero}/${totalBatches} • tentativa ${tentativa} ===`, lote)
          ...
          if (batchRes.status === 429 || batchData.rateLimited) {
            if (tentativa >= 3) {
              throw new Error(
                "O limite de leituras da API Olist/Tiny foi atingido repetidamente. Aguarde cerca de 1 minuto antes de iniciar outra sincronização."
              )
            }

            const retryMs = Math.max(61_000, Number(batchData.retryAfterMs) || 65_000)
            exibirToast(`Limite da API atingido. Aguardando ${Math.ceil(retryMs / 1000)}s para continuar...`, "error")
          ...
          alterados += Number(batchData.alterados) || 0
          preservados += Number(batchData.preservados) || 0
      ...
      const mensagem =
        `Estoque sincronizado: ${alterados} alterados, ${atualizados} conferidos, ` +
        `${preservados} preservados e ${falhas} falhas. ${processados}/${entries.length} produtos processados.`
          ...
          <div className="space-y-6 max-w-6xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Cadastre, edite e alinhe somente o estoque com o Olist/Tiny. O botão consulta o saldo real de cada produto e altera apenas o campo de estoque no site.</p>
              </div>
